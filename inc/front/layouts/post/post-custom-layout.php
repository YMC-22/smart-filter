...
while ($query->have_posts()) : $query->the_post();
	...
	do_action( "ymc_before_custom_layout_" . $target_id, $increment_post, $arrOptions );
	...
	$layouts .= '<div class="inform">'.esc_html__('Use a filter:','ymc-smart-filter').' 
                 ...
                 <span class="doc-text">add_filter("ymc_post_custom_layout", "func_custom_layout", 10, 5);</span>
                 <a target="_blank" href="https://github.com/YMC-22/smart-filter">'.esc_html__('See documentation.','ymc-smart-filter').'</a>
                 </div>';


	echo apply_filters('ymc_post_custom_layout_'.$target_id, $layouts, get_the_ID(), $filter_id, $increment_post, $arrOptions );

	$layouts = null;

	echo '</article>';

	do_action( "ymc_after_custom_layout_" . $target_id, $increment_post, $arrOptions );
